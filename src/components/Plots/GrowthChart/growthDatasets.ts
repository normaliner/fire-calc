...
export const growthDatasets = (calculator: CalculatorT) => {
	// const {
	...
	const totalMonthlyFixedExpenses = fixedExpenses.reduce(
		(acc, inv) => acc + inv.value,
		0
	);
	const totalMonthlyInflatingCosts = inflatingExpenses.reduce(
		(acc, inv) => acc + inv.value,
		0
	);
	for (let index = 0; index < months; index++) {
		...
		const growth = dataMonthlyAssets[index - 1] * (annualInvestmentRate / 12);
		dataMonthlyGrowth.push(growth);
		dataMonthlyIncome.push(
			dataMonthlyIncome[index - 1] * (1 + annualIncomeIncrease / 12)
		);
		dataMonthlyAssets.push(
			dataMonthlyAssets[index - 1] + totalMonthlyInvestments + growth
		);
		dataMonthlyFixedExpenses.push(totalMonthlyFixedExpenses);
	...
	const datasets = [
		...
		{
			label: 'Расходы',
			data: dataMonthlyFixedExpenses.map(
				(val, ind) => val + dataMonthlyInflatingCosts[ind]
			),
			borderWidth: 1,
		},
	];
